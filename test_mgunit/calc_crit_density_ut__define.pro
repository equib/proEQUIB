function calc_crit_density_ut::test_basic
  compile_opt strictarr
  
  base_dir = file_dirname(file_dirname((routine_info('calc_crit_density_ut__define', /source)).path))
  ...
  N_crit=calc_crit_density(temperature=temperature, $
                           ...
                           aij_data=s_ii_aij)
  result= long(N_crit[1]*10)
  assert, result eq 50078, 'incorrect result: %d', result
  
  return, 1
end

pro calc_crit_density_ut__define
  compile_opt strictarr

  define = { calc_crit_density_ut, inherits proEquibUTTestCase}
end
